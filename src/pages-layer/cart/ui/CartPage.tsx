      ...
      <main className="pt-28 sm:pt-32 pb-14 px-4 sm:px-6">
        <div className="max-w-6xl mx-auto">
          <div className="bg-white rounded-3xl p-5 sm:p-8 shadow-sm">
            <Link href="/catalog" className="text-[#1f8b85]/50 hover:text-[#1f8b85] text-sm inline-block mb-4">
              ← {toCatalog}
            </Link>
          ...
          {!hydrated ? (
            <div className="bg-white rounded-3xl mt-5 p-8 text-[#1f8b85]/60">Loading...</div>
          ) : cart.length === 0 ? (
            <div className="bg-white rounded-3xl mt-5 p-8 sm:p-10 text-center">
              <h2 className="text-xl font-bold text-[#1f8b85]">{emptyLabel}</h2>
              <p className="text-[#1f8b85]/60 mt-2">{emptyHint}</p>
              <Link
                href="/catalog"
                className="inline-flex mt-6 px-6 py-3 rounded-full bg-[#1f8b85] text-white font-semibold hover:bg-[#1f8b85]/85"
              >
                {toCatalog}
              </Link>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-5 mt-5">
                  ...
                  <article key={product.id} className="bg-white rounded-2xl p-4 sm:p-5">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <h3 className="font-bold text-[#1f8b85] truncate">
                          {lang === 'ru' ? product.nameRu : lang === 'uz' ? product.nameUz : product.nameEn}
                        </h3>
                        <p className="text-sm text-[#1f8b85]/60">{formatPrice(product.price, lang)}</p>
                      </div>
                    </div>

                    <div className="flex items-center justify-between mt-4 gap-3">
                      <div className="inline-flex items-center rounded-full border border-[#1f8b85]/20 overflow-hidden">
                        <button onClick={() => setQty(product.id, qty - 1)} className="px-3 py-1.5 text-[#1f8b85]/80 hover:bg-[#1f8b85]/8">−</button>
                        <span className="px-3 py-1.5 text-[#1f8b85] font-semibold min-w-[2.2rem] text-center">{qty}</span>
                        <button onClick={() => setQty(product.id, qty + 1)} className="px-3 py-1.5 text-[#1f8b85]/80 hover:bg-[#1f8b85]/8">+</button>
                      </div>
                      <button onClick={() => removeItem(product.id)} className="text-sm text-[#1f8b85]/50 hover:text-[#1f8b85]">
                        {removeLabel}
                      </button>
                    </div>
                  </article>
              ...
              <aside className="h-fit lg:sticky lg:top-28 bg-white rounded-2xl p-5 sm:p-6">
                <div className="flex items-center justify-between">
                  <span className="text-[#1f8b85]/60">{totalLabel}</span>
                  <span className="text-2xl font-extrabold text-[#1f8b85]">{formatPrice(total, lang)}</span>
                </div>
                <Link
                  href="/checkout"
                  className="w-full mt-5 inline-flex justify-center px-6 py-3.5 rounded-full bg-[#1f8b85] text-white font-bold hover:bg-[#1f8b85]/85 transition-colors"
                >
                  {checkoutLabel}
                </Link>
                <button
                  onClick={clearCart}
                  className="w-full mt-3 px-6 py-3 rounded-full border border-[#1f8b85]/20 text-[#1f8b85]/70 hover:text-[#1f8b85] hover:bg-[#1f8b85]/6"
                >
                  {clearLabel}
                </button>
              </aside>
